...
- **Security Hardening (Phase 4.1):** Includes **salted** SHA-256 hashing of PII in rate limiter logs (salt from `RateLimit:LogKeyHashSalt` config / `RATELIMIT_LOG_SALT` env, falling back to a per-process random 32-byte salt so the line is salted even if unset), CDN integrity checks (sha384 SRI + `crossorigin="anonymous"` on all 5 pinned CDN refs across `_Layout`/`_PopoutLayout`/`_ModernLayout`), OpenTelemetry exporter bumped 1.12.0 → 1.15.3 to clear NU1902, sanitization of `Cookie`/`Authorization`/`Set-Cookie` header tags on AspNetCore + HttpClient OTel spans via `EnrichWithHttp{Request,Response}` callbacks, and explicit cookie security settings (`HttpOnly=true`, `SameSite=Lax`, `SecurePolicy=Always` in Production / `SameAsRequest` in Development, `ExpireTimeSpan=8h`, `SlidingExpiration=true`). Coverage: xUnit `PostgresLoginRateLimiterRedactionTests` injects a broken `IServiceScopeFactory` and asserts the captured warning contains the salted 12-hex `keyHash` but never the raw IP/username/partition key, plus determinism-per-salt and difference-across-salts. Smoke `tests/smoke_phase4.spec.js` adds: every CDN tag rendered on `/Account/Login` carries sha384+crossorigin; file-level SRI ratchet across the three layouts (asserts total==5 pinned refs); auth cookie posture after `login()` (HttpOnly, SameSite=Lax, finite expires); production-simulation cookie posture (POST `/Account/Login` with `X-Forwarded-Proto: https` asserts the issued `.AspNetCore.Cookies` carries `Secure`, `HttpOnly`, `SameSite=Lax`, and a finite `expires`/`max-age`); rate-limiter source still hashes the partition key and never reverts to `partitionKey.Substring(0, 24)`.

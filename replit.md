...
- **Phase 4 Production Readiness:** Closes the three gaps surfaced after the Phase 3 deploy.
  ...
  - **Smoke regression** — `tests/smoke_phase4.spec.js` locks the three contracts: 130-burst trips ≥15 × 429, all four security headers present on `/_live` and `/Account/Login` with CSP `frame-ancestors` containing `'self'`, and the OTel pipeline boots cleanly with no exporter configured.

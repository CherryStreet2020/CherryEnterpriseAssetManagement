#!/usr/bin/env python3
"""
GATE: CIP TotalCosts Reconciliation
Verifies that CipProject.TotalCosts matches SUM(CipCosts.Amount) for every project.
FAIL conditions:
  1) ABS(stored_total - computed_total) > 0.01
  2) stored_total > 0 AND cost_count = 0
"""
import os, sys, json
import psycopg2
...
print("=" * 80)
print("GATE: CIP TotalCosts Reconciliation")
print("=" * 80)
...
for row in rows:
    ...
    if discrepancy > 0.01:
        status = "FAIL"
        fail_reasons.append(f"discrepancy={discrepancy:.2f}")

    if stored > 0 and count == 0:
        status = "FAIL"
        fail_reasons.append(f"stored={stored:.2f} but cost_count=0")
...
audit = {
    "gate": "cip_totalcosts_reconciles",
...
output_dir = os.environ.get("GATE_OUTPUT_DIR", "proof/cip_integrity/after")
os.makedirs(output_dir, exist_ok=True)

with open(os.path.join(output_dir, "cip_totalcosts_audit.json"), "w") as f:
    json.dump(audit, f, indent=2)

with open(os.path.join(output_dir, "gate_cip_totalcosts_reconciles.txt"), "w") as f:
    f.write(f"GATE: CIP TotalCosts Reconciliation\n")

...
{
    // ADR-012 / PR #119.1 (renamed in #119.1.1) — Unified Work Orders:
    // top-level classification + external-system linkage on MaintenanceEvent.
    //
    // Adds three columns:
    //
    //   Classification        smallint NOT NULL DEFAULT 0   (WorkOrderClassification enum)
    //   ExternalWorkOrderId   varchar(64) NULL
    //   ExternalSource        varchar(32) NULL
    //
    // ("Classification" instead of "Category" because a legacy
    // WorkOrderCategory enum already exists on the WorkOrderType lookup
    // table — different concept; see PR #119.1.1.)
    //
    // Backfill semantics: every existing MaintenanceEvent row becomes
    // Classification=Maintenance (0). Lossless. The .NET class default
    // also matches so new inserts that don't specify Classification get
    // the right value.
    //
    // Indexes:
    //   - IX_MaintenanceEvents_Classification — drives the Plant Floor +
    //     WorkQueue filter chips ("show only Production WOs", etc.)
    ...
    public partial class AddWorkOrderCategoryAndExternalSource : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            ...
            migrationBuilder.Sql(@"
                CREATE INDEX IF NOT EXISTS ""IX_MaintenanceEvents_Classification""
                ON ""MaintenanceEvents"" (""Classification"");
            ");
        ...
        protected override void Down(MigrationBuilder migrationBuilder)
        {
            ...
            ");
            migrationBuilder.Sql(@"
                DROP INDEX IF EXISTS ""IX_MaintenanceEvents_Classification"";
            ");
            migrationBuilder.Sql(@"
                ALTER TABLE ""MaintenanceEvents""

...
public class ReceivingExceptionLoggingTests
{
    ...
    public async Task SaveFailure_GenericException_LogsErrorAndSurfacesCorrelationInTempData()
    {
        ...
        var receivingPosting = new Abs.FixedAssets.Services.Receiving.ReceivingPostingService(db, tenant, glResolver, outbox, new PassthroughIdempotencyMediator(), new Abs.FixedAssets.Tests.TestHelpers.NullChainOfCustodyService(), new Abs.FixedAssets.Tests.TestHelpers.NullReceiptToJobService(), NullLogger<Abs.FixedAssets.Services.Receiving.ReceivingPostingService>.Instance);
        var page = new Abs.FixedAssets.Pages.Receiving.ReceiveModel(
            db, new AlwaysEnabledModuleGuard(), tenant, lookup, new AllowAllPeriodGuard(), logger, cipAutoCost, receivingPosting, outbox);

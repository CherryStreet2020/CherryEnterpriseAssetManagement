...
public class CatalogPageTests
{
    ...
    public void OnGet_ProducesDescriptorForEveryRegisteredEvent()
    {
        ...
        // 5 baseline + 3 AP + 3 Asset + 3 PO/Receiving + 1 CIP = 15.
        Assert.Equal(15, page.Events.Count);
        ...
        Assert.Contains(page.Events, e => e.EventType == "asset.created" && e.Version == 1);
        Assert.Contains(page.Events, e => e.EventType == "asset.improved" && e.Version == 1);
        Assert.Contains(page.Events, e => e.EventType == "asset.disposed" && e.Version == 1);
        Assert.Contains(page.Events, e => e.EventType == "po.approved" && e.Version == 1);
        Assert.Contains(page.Events, e => e.EventType == "po.received" && e.Version == 1);
        Assert.Contains(page.Events, e => e.EventType == "item.received" && e.Version == 1);

...
| **Sprint 15.3 — Purchasing Cascade Wave 3 (Purchasing Command Center)** ⭐ NEXT UP | PRs 10-15 of the 20-PR cascade. The §21 Purchasing Control Center on top of Wave 1+2 substrate. **PR-10:** `IPurchasingControlCenterService` + Supply Demand queue (13 queue types from §21, exception lane, KPI band). **PR-11:** Purchasing CC page — Supply Demand + Buy-to-Job tabs (composes Cockpit primitives; flip `PURCHASING` to `IsLive: true`). **PR-12:** Purchasing CC — Subcontract + Vendor WIP tabs (renders `_CockpitSubcontractPanel` from PR-9). **PR-13:** Purchasing CC — POs + Expedites + Approvals tabs. **PR-14:** Auto-PO Creation Rules (`IAutoPurchaseService` per §16 — auto-create candidate triggers + auto-block rules) + Demand Consolidation (`IDemandConsolidationService` per §17 — 6 consolidation modes preserving demand traceability via PurchaseOrderLineDemandLink). **PR-15:** Buyer Recommendation Engine (`IPurchasingRecommendationService` per §18 — 10+ recommendation patterns; wire into CC Supply Demand queue as "Suggested Action" column). ~52h estimated. | #408-#413 (placeholder) | 0 | 6 | — |

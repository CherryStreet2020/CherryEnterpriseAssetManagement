...
| **Sprint 15.2 — Purchasing Cascade Wave 2 (Subcontract Flow)** ⭐ NEXT UP | PRs 6-9 of the 20-PR cascade. Complete the 8-step subcontract operation flow from spec §5. **PR-6:** SubcontractShipment + SubcontractShipmentLine + SubcontractReceipt + SubcontractReceiptLine entities with lot/serial/revision traceability and 9 receipt scenarios from §11 (full good, partial, with inspection, rejected, vendor scrap, short, over, cert missing, wrong revision). **PR-7:** `ISubcontractFlowService` — full 8-step orchestrator from §5 (PRO release → demands → PO/req → gate on prior op → ship WIP → track vendor status → receive subcontract PO + return WIP → next op + readiness check). Wires into existing `IOperationReadinessService` (B8 PR-PRO-7) + cost engine. **PR-8:** Subcontract cost elements (§12) into CostTransaction engine — service cost, freight, packaging, expediting, cert fees, scrap/rework, PPV, invoice variance. New CostTransactionType + GlAccountKind additions. **PR-9:** Subcontract panel on PRO Cockpit — per-operation view (supplier, PO, status, qty shipped/at-vendor/received/rejected, due date, days late, cert/inspection status, actions). 15 non-negotiable validations from §24 implemented as guards. ~30h estimated. | #401-#404 (placeholder numbers; 401-402 already used by housekeeping PRs) | 0 | 4 | — |
| **Sprint 15.3 — Purchasing Control Center (Wave 3)** | PRs 10-15 of the 20-PR cascade. `IPurchasingControlCenterService` + 13 queue tabs from §21 + auto-PO creation + demand consolidation + buyer recommendation engine. ~52h. | TBD | 0 | 6 | — |
| **Sprint 15.4 — Purchasing Polish (Wave 4)** | PRs 16-20 of the 20-PR cascade. PO Acknowledgment + PO Amendment + Vendor Performance + 3-Way Match + RFQ/Quote flow. ~34h. | TBD | 0 | 5 | — |
...
## 📋 Master Plan Audit 2026-05-27 (CANONICAL — supersedes 2026-05-24)

...
| **S2-5** | Disposal does not emit outbox event | `Pages/Assets/Dispose.cshtml.cs:160-217` | Writes Asset disposal fields + JournalEntry + (optional) `PartialDisposal`, but does not enqueue `asset.disposed`. | Partner integrations get no signal on disposal. Voice/mobile reporting is blind. | ~20 LOC. |
| **S2-6** | Missing outbox events for the real domain transitions | `Services/Webhooks/Events/*.cs` (only 5 typed events) | Per CLAUDE.md outbox principle, every business state change must produce an event. Today only WO-created/closed and lesson-saved emit. Missing: `po.approved`, `po.received`, `invoice.approved`, `invoice.posted`, `invoice.paid`, `cip.capitalized`, `asset.created`, `asset.disposed`, `depreciation.posted`. | Partner integration story is incomplete. | ~250 LOC: define typed records + add `EnqueueAsync` calls at each SaveChanges site. |

...
| # | Title | Files | What's wrong | Impact | Fix sizing |
|---|---|---|---|---|---|
| ~~**S2-1**~~ | ~~Two parallel PM scheduler stacks~~ ✅ Closed via [#78](https://github.com/CherryStreet2020/CherryEnterpriseAssetManagement/pull/78) | `GenerateEventsFromSchedulesAsync` now scopes by `Asset.CompanyId IN VisibleCompanyIds` and is marked `[Obsolete]` pointing to `PMSchedulerService` | — | — | — |
| ~~**S2-2**~~ | ~~PM has no meter-based recurrence~~ ✅ Foundation closed via [#79](https://github.com/CherryStreet2020/CherryEnterpriseAssetManagement/pull/79) | `IPMSchedulerService.ProjectMeterCrossingDateAsync` ships with linear-velocity projection and a structured `MeterCrossingProjection` result. Wiring into `PreviewDueAsync` / `GenerateDueAsync` is the next slice when a meter-driven workflow needs to ship | — | — | — |
